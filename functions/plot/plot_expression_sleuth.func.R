...
##log2srpbs = data.frame, rownames as genes
...
##genes = those to plot, must match in rownames(input)
...
plot_expression_so <- function(so, genes, mapgenes, conds, condcol, factors, tag, genemap=NULL){
    ...
    if(class(so)=="sleuth"){
      log2_obs_norm_filt_df <- dcast(so$obs_norm_filt,
                                       target_id ~ sample,
    ...
    if(class(so)!="sleuth"){
      log2_obs_norm_filt_df <- so
    }
    log2srpb_genes <- log2_obs_norm_filt_df
